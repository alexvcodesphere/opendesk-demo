  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        ...
        {step === 'success' && (<div className="py-8 text-center"><div className="w-16 h-16 mx-auto rounded-full bg-success/20 flex items-center justify-center"><Check className="w-8 h-8 text-success" /></div><h3 className="mt-4 text-xl font-semibold">Project Created!</h3><p className="mt-2 text-muted-foreground">Your project &quot;{projectName}&quot; has been created with <span className="text-primary font-medium">{enabledProviders.length} application{enabledProviders.length !== 1 ? 's' : ''}</span>.</p><div className="mt-6 flex gap-3 justify-center"><Button variant="outline" onClick={handleClose}>Stay Here</Button><Button onClick={handleGoToProject}>Go to Project</Button></div></div>)}
